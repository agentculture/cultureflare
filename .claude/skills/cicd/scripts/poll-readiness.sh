...
# to MERGED/CLOSED). Override the default via CULTUREFLARE_PR_REVIEWERS=...
...
#
# Detection heuristics (mirror cultureflare's `poll` skill):
...
Defaults: --max-iters 30, --interval 60, --require qodo
  (set CULTUREFLARE_PR_REVIEWERS to override the default --require list)
...
INTERVAL=60
PR_NUMBER=""
REQUIRE="${CULTUREFLARE_PR_REVIEWERS:-qodo}"
