#!/usr/bin/env bash
# cultureflare cicd workflow entry point (vendored from steward 0.7.0; the
# AgentCulture-standard skill name for the PR-review lane).
#
# Subcommands:
...
#                           threads. Tunables: CULTUREFLARE_PR_AWAIT_ITERS (default 30),
#                           CULTUREFLARE_PR_AWAIT_INTERVAL (default 60),
#                           CULTUREFLARE_PR_REVIEWERS (default "qodo").
...
case "$cmd" in
    ...
    await)
        ...
        ITERS="${CULTUREFLARE_PR_AWAIT_ITERS:-30}"
        INTERVAL="${CULTUREFLARE_PR_AWAIT_INTERVAL:-60}"
