...
description: >
  ...
  defaults records to this agent's PERSONAL, PRIVATE scope (`--scope cultureflare
...
scope:** the default personal scope is **private** (`--scope cultureflare
...
- `--scope NAME` / `--visibility public|private` — record scope. **The wrapper
  ...
  (here, `cultureflare`). Private records are served only to a recall in the same
